...
class BasicBlock(SymExecObject):
    """
    继承自 SymExecObject 类。该类表示程序的基本块，它由一系列指令组成
    _instructions 表示指令序列，
    loops 表示该基本块中包含的循环，
    visited 用于标识该基本块是否已经被 DFS 访问过
    exec_count 表示该基本块被执行的次数
    _nexts 和 _prevs 分别表示该基本块的下一个和上一个基本块
    sub_blocks 表示该基本块中包含的子基本块
    """

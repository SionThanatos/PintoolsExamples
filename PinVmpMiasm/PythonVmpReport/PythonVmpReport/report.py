...
def report_html (side_bar, main_content):
	"""
	函数生成 HTML 页面的代码，包括了侧边栏、顶部菜单和主要内容区域
	"""
...
def header_h(title):
	"""
	生成一个 HTML 标题，标题级别为二级标题（h2），样式类名为 ui dividing header
	"""
...
def gen_ins_info(bm):
	...
	addrs = list(bm.instructions.keys())
	addrs.sort()
	...
	for addr in addrs:
		...
		#在 Python 3 中，bytes 类型不再拥有 encode() 方法，因为 bytes 对象本身就是字节序列，而不需要编码
		#rows += td_fmt % ( ins.bytes.encode('hex').upper() )
		rows += td_fmt % ins.bytes.hex().upper()
...
def gen_graph(bm):
	"""
	该函数会调用 bm.gen_bbl_graph() 方法来生成三个图像文件，
	分别是以 .jpg 格式渲染的基本块控制流图和以 .svg 格式渲染的三个层次的控制流图。
	最后，该函数会返回一个包含 HTML 代码段的字符串，用于显示三个 SVG 图像
	"""
...
def gen_handler(bm):
	"""
	根据传入的 bm 对象生成一组 HTML 元素，表示所有处理器对象的详细信息
...
def gen_report(bm):
	...
	open(config.REPORT_PATH, 'wb').write(text.encode('utf-8'))
